...
def generate_response(message: str, is_verify: bool = False) -> ChatResponse:
    """
    기존 generate_response를 실제 AI로 교체
    """
    try:
        # Azure 클라이언트들 가져오기 (이미 초기화된 인스턴스)
        ...
        # AI 응답 생성
        result = get_text_completion_result(
            Query=query,
            OAI_client=chat_client,
        ...
        # ChatResponse 객체로 변환
        return ChatResponse(
            message=result.get("response", "응답을 생성할 수 없습니다."),
            keywords=result.get("resp_keywords", []),
