...
# develop 브랜치 호환을 위한 기존 응답 생성 함수 (백업용)
def generate_response_legacy(message: str):
    """기존 develop 브랜치 호환용 응답 생성"""
    # 기존 서비스의 간단한 응답 생성 로직
    class LegacyResponse:
        def __init__(self, message, keywords=None, sources=None):
            self.message = message
            self.keywords = keywords or ["키워드"]
            self.sources = sources or ["출처"]
    
    return LegacyResponse(
        message=generate_simple_response(message),
        keywords=["조선왕조실록", "역사"],
        sources=["한국사 DB"]
    )
...
async def historical_chat(request: ChatRequest, db: Session = Depends(get_db)):
    ...
    if request.chat_mode is not None:
        ...
        # 기존 방식 호환: is_verify 기반
        chat_mode = "verification" if request.is_verify else "creative"
        strictness = request.strictness or 2  # 기존 기본값
    ...
    try:
        ...
        # 2. 문서 검색
        docs = search_documents(keywords, top_k=request.top_n_documents or 5)
